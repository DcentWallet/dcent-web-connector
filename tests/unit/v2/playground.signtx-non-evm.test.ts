...
describe('_substituteAlgorandSender: placeholder → wallet address', () => {
  ...
  it('T-U-NEVM-FAMILY-SUB-04: _substituteSenderByFamily — xrp family → XRP Account 치환', () => {
    const tx = { TransactionType: 'Payment', Account: 'raJ1Aqkhf19P7cyUc33MMVAzgvHPvtNFC', Destination: RECIPIENT, Amount: '1000000' }
  ...
  it('T-U-NEVM-FAMILY-SUB-05: _substituteSenderByFamily — xahau family → XRP Account 치환 (ripple 로직 공유)', () => {
    const tx = { TransactionType: 'Payment', Account: 'raJ1Aqkhf19P7cyUc33MMVAzgvHPvtNFC', Destination: RECIPIENT }
...
describe('_substituteXrpAccount: placeholder → wallet r-address', () => {
  ...
  const RECIPIENT = 'rGZQKj1U9fy21xRjrAtYCniUarwxFRrzmi'
  ...
  it('T-U-NEVM-XRP-SUB-01: XRPL 표준 Payment — Account 치환, Destination/Amount/Fee/Sequence 보존', () => {
    const tx = { TransactionType: 'Payment', Account: 'raJ1Aqkhf19P7cyUc33MMVAzgvHPvtNFC', Destination: RECIPIENT, Amount: '1000000', Fee: '12', Sequence: 1, Flags: 0 }
    const out = sub(tx)
    expect(out.Account).toBe(WALLET)
    expect(out.Destination).toBe(RECIPIENT)
    expect(out.Amount).toBe('1000000')
  ...
  it('T-U-NEVM-XRP-SUB-02: wm-internal {sender, ...} shape → sender 치환', () => {
    const tx = { sender: 'raJ1Aqkhf19P7cyUc33MMVAzgvHPvtNFC', recipient: RECIPIENT, amount: '1000000' }
    const out = sub(tx)
    expect(out.sender).toBe(WALLET)
    expect(out.recipient).toBe(RECIPIENT)
  })
  ...
  it('T-U-NEVM-XRP-SUB-04: 원본 객체 mutation 금지 (deep clone)', () => {
    ...
    const out = sub(tx)
    expect(out).not.toBe(tx)
    expect(tx.Account).toBe('raJ1Aqkhf19P7cyUc33MMVAzgvHPvtNFC')
  })
  ...
  it('T-U-NEVM-XRP-SUB-06: empty wallet address → no-op (원본 보존)', () => {
    const tx = { TransactionType: 'Payment', Account: 'raJ1Aqkhf19P7cyUc33MMVAzgvHPvtNFC' }
    const out = sub(tx, '')
    expect(out.Account).toBe('raJ1Aqkhf19P7cyUc33MMVAzgvHPvtNFC')
  })
})
